			   ...
			   var thisBuild = config.build[i];
				...
				if (typeof thisBuild.components === 'object') {
					for (var i2 = 0, len2 = thisBuild.components.length; i2 < len2; i2++) {
					   var data = JSON.parse(fs.readFileSync(path.join(v.path.component, thisBuild.components[i2], 'package.json')));
						// Catch
						...
						if (typeof data.rocket.build == 'object') {
							for (var i3 = 0, len3 = bower.main.length; i3 < len3; i3++) {
								addFileToReturn(thisBuild.name, path.join(v.path.project.component, thisBuild.name, bower.main[i3]));
							}
						} else if (typeof data.rocket.build === 'string') {
							addFileToReturn(thisBuild.name, path.join(v.path.component, thisBuild.name, data.rocket.build));
						}
					}
				}
				// SASS

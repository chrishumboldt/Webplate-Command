...
module.exports = {
   execute: {
      css: function (obj, buildRoot) {
         ...
         // Variables
         const outputPath = (Rocket.exists(obj.outputPath) && Rocket.exists(obj.outputPath.css)) ? obj.outputPath.css : v.build.outputPath.css;
         const subExtension = (Rocket.exists(obj.subExtension)) ? obj.subExtension : v.build.subExtension;
         const nameFull = obj.name + subExtension + '.css';

         let fileData = '';
         const fileOutputPath = path.join(outputPath, nameFull);
         let renderData = (v.dirName() === 'rocket') ? '@import "engine/build/sass/import.scss";' : '';
      ...
      js: function (obj, buildRoot) {
         ...
         // Variables
         let jsFile = '';
         const outputPath = (Rocket.exists(obj.outputPath) && Rocket.exists(obj.outputPath.js)) ? obj.outputPath.js : v.build.outputPath.js;
         const subExtension = (Rocket.exists(obj.subExtension)) ? obj.subExtension : v.build.subExtension;

         const nameFull = obj.name + subExtension + '.js';
         const filePath = path.join(outputPath, nameFull);
         ...
         for (let i = 0, len = obj.js.length; i < len; i++) {
            ...
            switch (ext) {
               case 'coffee':
                  const compiledCode = coffeeScript.compile(fileContents);
                  js = babel.transform(compiledCode, v.options.babel).code;
                  break;
               ...
               case 'js':
                  if (/.min.js$/.test(obj.js[i])) {
                     ...
                     jsFile += babel.transform(fileContents, v.options.babel).code;
                  }
                  break;
         ...
         // Continue
         fs.writeFile(filePath, jsFile, (error) => {
            if (error) {
               Rocket.log(colour.error(error));
               return false;
            }
   ...
   getCockpit: function (callback) {
      ...
      cockpit.read((json) => {
         ...
         // Continue
         const buildRoot = (Rocket.is.string(json.buildRoot)) ? json.buildRoot : v.build.root;
   ...
   init: function (type) {
      ...
      self.getCockpit((json) => {
         ...
         // Bust cache
         if (v.dirName() === 'rocket') cache.bust(false);
      });
   },
   ...
   validate: {
      build: function (build, type) {
         ...
         switch (type) {
            ...
            case 'js':
               if (!Rocket.is.array(build.js)) return false;
               if (build.js.length < 1) return false;
               break;

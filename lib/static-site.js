...
module.exports = {
	build: function () {
		...
		var renderPages = function (staticSite) {
			Rocket.log(staticSite);
		};

		// Execute
		fs.stat(staticPath, function (error) {
			if (error) {
				Rocket.log(colour.command('Folder: ') + colour.action('project/build/static...') + colour.command('not found'));
				Rocket.log(colour.text('The folder either needs to be created or you are currently not in the Rocket folder at this time.'));
			} else {
				Rocket.log(colour.title('Building your static site...'));

/**
 * NOTE
 * This is currently in the process of being deprecated.
 * It is not longer being used by Rocket Command.
 * Do not modify this file as it will soon be removed.
**/
...
module.exports = {
	add: function(component) {
		if (v.dirName() === 'rocket') {
			Rocket.log(colour.title('Downloading the package...'));
			var child = shell.exec('npm install ' + component, {
				async: true
			});
			// Output
			...
			child.on('close', function() {
				var packageJson = path.join('node_modules', component, 'package.json');
				fs.readFile(packageJson, function(error, data) {
					Rocket.log('');
					Rocket.log(colour.command('Component: ') + colour.action(component) + colour.title(' has been added. Woohoo!'));
					Rocket.log(colour.text('You can view it under the node_modules directory.'));
					Rocket.log('');
					if (!error) {
						...
						fs.writeFile(packageJson, JSON.stringify(data, null, '\t'), function (error) {
							if (error) {
								Rocket.log(colour.error(error));
							} else {
								Rocket.log('');
								Rocket.log(colour.warning('Warning: ') + colour.text('No ') + colour.action('rocket') + colour.text(' parameter found in ') + colour.action('node_modules/' + component + '/package.json') + colour.text(' file.'));
								Rocket.log(colour.command('Parameter: ') + colour.action('rocket...') + colour.command('created'));
								Rocket.log(colour.name('Note: ') + colour.text('Check the package.json "rocket" parameter.'));
								Rocket.log(colour.name('Note: ') + colour.text('Check that the "production" property references the production files required.'));

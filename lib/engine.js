...
module.exports = {
	build: {
		all: function() {
			if (v.dirName() === 'rocket') {
				Rocket.log(colour.title('Rebuilding the engine...'));
				this.css();
				this.scripts();
				this.start();
				this.touch();
				...
				if (error) {
					...
					fs.writeFile(path.join(v.path.engine.css, 'styles.min.css'), css, function(error) {
						if (error) {
							...
							Rocket.log(colour.command('CSS: ') + colour.action('engine/css/styles.min.css...') + colour.command('successful'));
						}
					});
				}
			});
				...
				if (error) {
					...
					fs.writeFile(path.join(v.path.engine.css, 'styles-light.min.css'), css, function(error) {
						if (error) {
							...
							Rocket.log(colour.command('CSS: ') + colour.action('engine/css/styles-light.min.css...') + colour.command('successful'));
						}
					});
				}
			});
		...
		scripts: function() {
			...
			fs.writeFile(path.join(v.path.engine.js, 'scripts.min.js'), engineJS.code, function(error) {
				if (error) {
					...
					Rocket.log(colour.command('JS: ') + colour.action(path.join(v.path.engine.js, 'scripts.min.js...')) + colour.command('successful'));
				}
			});
			...
			fs.writeFile(path.join(v.path.engine.js, 'scripts-light.min.js'), engineJSLight.code, function(error) {
				if (error) {
					...
					Rocket.log(colour.command('JS: ') + colour.action(path.join(v.path.engine.js, 'scripts-light.min.js...')) + colour.command('successful'));
				}
			});
		},
		start: function() {
			...
			fs.writeFile(path.join('launch.js'), engineStartJS.code, function(error) {
				if (error) {
					Rocket.log(colour.error(error));
				} else {
					Rocket.log(colour.command('JS: ') + colour.action('launch.js...') + colour.command('successful'));
				}
			});
		},
		touch: function() {
			...
			fs.writeFile(path.join(v.path.engine.js, 'touch.min.js'), engineTouchJS.code, function(error) {
				if (error) {
					...
					Rocket.log(colour.command('JS: ') + colour.action(path.join(v.path.engine.js, 'touch.min.js....')) + colour.command('successful'));
				}
			});
		}
	}

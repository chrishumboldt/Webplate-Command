				...
				if ($error) {
					...
					fs.writeFile(path.join($v.path.engine.css, 'styles.min.css'), $css, function($error) {
						if ($error) {
							...
							web.log(colour.command('CSS: ') + colour.action('engine/styles.min.css...') + colour.command('successful'));
						}
					});
				}
			});
		...
		scripts: function() {
			...
			fs.writeFile(path.join($v.path.engine.js, 'scripts.min.js'), $engineScriptJS.code, function($error) {
				if ($error) {
					...
					web.log(colour.command('JS: ') + colour.action('engine/scripts.min.js...') + colour.command('successful'));
				}
			});
		},
		start: function() {
		...
		touch: function() {
			...
			fs.writeFile(path.join($v.path.engine.js, 'touch.min.js'), $engineTouchJS.code, function($error) {
				if ($error) {
					...
					web.log(colour.command('JS: ') + colour.action('engine/touch.min.js...') + colour.command('successful'));
				}
			});
		}
	}
